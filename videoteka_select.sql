...
--zakasninu prikazati (broj dana)
SELECT c.*, AS Zakasnina
FROM clanovi c
JOIN posudba p ON c.id = p.clan_id
WHERE IFNULL()

--kolicina filmova po mediju, za svaki film

SELECT f.naslov, m.tip, count(f.id) AS 'Broj kopija'
FROM kopija k
